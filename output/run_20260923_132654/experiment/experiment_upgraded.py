...
# =========================================================================
# PROPOSED METHOD: ADAPTIVE TOPOLOGICAL HARMONIZATION (ATH) ARCHITECTURE
# =========================================================================
class AdaptiveTopologicalHarmonizationLayer(nn.Module):
    """
    Adaptive Topological Harmonization (ATH) module:
    Harmonizes representation gradients using adaptive layer-wise normalization
    and high-frequency preservation regularizer.
    """
    def __init__(self, hidden_dim: int):
        super(AdaptiveTopologicalHarmonizationLayer, self).__init__()
    ...
    def forward(self, h):
        ...
        # Adaptive layer-wise normalization with learnable scale/bias
        h_norm = self.norm(h)
        return h_norm * self.scale + self.bias


class ATHTemporalForecastingModel(nn.Module):
    """Mô hình dự báo chuỗi thời gian phụ tải điện năng tích hợp ATH."""
    def __init__(self, input_dim: int = 4, hidden_dim: int = 48, num_layers: int = 2):
        super(ATHTemporalForecastingModel, self).__init__()
        self.gru = nn.GRU(
        ...
        self.ath = AdaptiveTopologicalHarmonizationLayer(hidden_dim)
    ...
    def forward(self, x):
        ...
        # Apply Adaptive Topological Harmonization
        harmonized_repr = self.ath(last_step)
        
        h = F.relu(self.fc1(harmonized_repr))
        pred = self.fc2(h)
        return pred, harmonized_repr
...
# =========================================================================
def main():
    parser = argparse.ArgumentParser(description="Household Energy Consumption Forecasting Benchmark with ATH")
    ...
    parser.add_argument("--lambda_reg", type=float, default=0.01, help="Trọng số điều chuẩn topological regularization")
    ...
    model = ATHTemporalForecastingModel(input_dim=4, hidden_dim=48, num_layers=2).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for b in range(n_batches):
            ...
            optimizer.zero_grad()
            preds, harmonized_repr = model(batch_x)
            
            # Mathematical Formulation: L = L_MSE + lambda * L_reg
            # L_reg prevents over-smoothing by preserving high-frequency variance/components in representation space
            mse_loss = criterion(preds, batch_y)
            high_freq_reg = torch.mean(torch.var(harmonized_repr, dim=0))
            loss = mse_loss - args.lambda_reg * high_freq_reg
    ...
    print(
        f"[EXPERIMENT COMPLETED (ATH)] Test MSE: {final_info['best_test_mse']:.4f}, "
